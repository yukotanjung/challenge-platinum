...
function createLoginToken(server, loginDetails, done) {
  ...
  describe('Orders', () => {
    it('GET / => list of order', () => {
      return (
        createLoginToken(app, { username: 'yukotanjung', password: '123455678' }, function(header) {
        request(app)
        .get('/orders/order')
  ...
  it('UPDATE /update-status-order => update status of order', () => {
      return createLoginToken(app, { username: 'yukotanjung', password: '123455678' }, function(header) {
